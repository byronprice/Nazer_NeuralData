...
for numBases = 1:10
    ...
    while iter<maxIter || difference>tolerance
        for jj=1:numParameters
            ...
            tempLikelihood = -N/2*(d*log(2*pi)+log(det(C))+trace(C\S));
        ...
        for jj=1:numParameters
            ...
            for kk=2:lineN
                tempParams = saveParameters;
                tempParams(jj) = tempParams(jj)+sign(gradient(jj))*lineSteps(kk);
                B = reshape(tempParams(1:end-1),[historyParams,numBases]);
                sigmasquare = tempParams(end);
                W = H*B;
                C = var(W(:))+sigmasquare.*eye(d);
                lineLikelihood(kk) = -N/2*(d*log(2*pi)+log(det(C))+trace(C\S));
            end
        ...
        iter = iter+1;
        %     plot(iter,loglikelihood,'.');pause(1/100);
    end
    ...
    AIC(numBases) = dev+2*(historyParams*numBases+1);
